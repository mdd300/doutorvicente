...
jQuery(document).ready(function($){
	...
	//slider
	$('.slider_content').first().css('display', 'block');
	...
	{
		//google map
		var coordinate = new google.maps.LatLng(-23.575500999999999, -46.665528000000002);
		var mapOptions = {
			zoom: 12,
			center: coordinate,
	...
	$('#newsletter_form').submit(function (event) {
		...
		var sendButton = $('#newsletter_form input[type="submit"]');

		sendButton.attr('disabled', true);
		sendButton.val('Enviar...');
		...
		$.ajax({
			...
			success: function (retorno) {
				sendButton.val('Enviar');
				$('#message-newsletter').text(retorno.message);
			...
			error: function (XMLHttpRequest, textStatus, errorThrown) {
				$('#message-newsletter').html('Tivemos um problema no envio, tente novamente mais tarde.');
			}
		});
